...
class RedisManager(private val plugin: HuHoBotVelocity) {
    ...
    fun connect(host: String, port: Int, password: String?) {
        try {
            ...
            plugin.logger.error("Redis 连接失败: ${e.message}")
            jedisPool = null
        }
    }
        ...
        subscriberJob = CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                Jedis(host, port).use { jedis ->
                    ...
                    pubSub = object : JedisPubSub() {
                        override fun onMessage(channel: String, message: String) {
                            plugin.logger.debug("收到回调消息: $message")
                            commandCallback?.handleCallback(message)
                        }

                        override fun onSubscribe(channel: String, subscribedChannels: Int) {
                            plugin.logger.debug("已订阅回调频道: $channel")
                        }

                        override fun onUnsubscribe(channel: String, subscribedChannels: Int) {
                            plugin.logger.debug("已取消订阅频道: $channel")
                        }
                    }
                ...
                if (e !is CancellationException) {
                    plugin.logger.error("Redis 回调监听器异常: ${e.message}")
                }
            }
        }
    }
    ...
    fun sendCommand(serverName: String, command: String): Boolean {
        return try {
            jedisPool?.resource?.use { jedis ->
                ...
                plugin.logger.debug("已发送命令到 $serverName: $command")
                true
            } ?: run {
                plugin.logger.warn("Redis 未连接，无法发送命令")
                false
            }
        } catch (e: Exception) {
            plugin.logger.error("发送命令失败: ${e.message}")
            false
        }
    }
    ...
    fun sendCommandWithCallback(serverName: String, taskId: String, command: String): Boolean {
        return try {
            jedisPool?.resource?.use { jedis ->
                ...
                jedis.publish(commandChannel, message)
                plugin.logger.debug("已发送回调命令到 $serverName [taskId=$taskId]: $command")
                true
            } ?: run {
                plugin.logger.warn("Redis 未连接，无法发送命令")
                false
    ...
    fun publish(type: String, data: String): Boolean {
        return try {
            ...
            plugin.logger.error("发布消息失败: ${e.message}")
            false
        }
    }
