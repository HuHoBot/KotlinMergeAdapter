...
import cn.huohuas001.huhobot.nkmot.tools.SetController
import cn.nukkit.utils.Config

class QueryWhiteList(private val plugin: HuHoBotNkMot): BaseEvent() {
    override fun run(): Boolean {
        val originWhiteList: Config = plugin.server.whitelist
        val whiteList: Set<String> = SetController.convertToPlayerNames(originWhiteList)
        val whitelistNameString = StringBuilder()
        val rBody: com.alibaba.fastjson2.JSONObject = com.alibaba.fastjson2.JSONObject()
        if (mBody.containsKey("key")) {
            ...
            whitelistNameString.append("查询白名单关键词:").append(key).append("结果如下:\n")
            val filterList: List<String> = SetController.searchInSet(whiteList, key)
            ...
            val splitedNameList: List<List<String>> = SetController.chunkSet(whiteList, 10)
