package cn.huohuas001.huhobot.common.redis
...
class RedisManager(private val plugin: HuHoBotProxy) {
    ...
    fun connect(host: String, port: Int, password: String?) {
        try {
            ...
            plugin.log_info("Redis 连接成功: $host:$port")
            ...
            plugin.log_error("Redis 连接失败: ${e.message}")
            jedisPool = null
        }
    }
        ...
        subscriberJob = CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (e !is CancellationException) {
                    plugin.log_error("Redis 回调监听器异常: ${e.message}")
                }
            }
        }
    }
    ...
    fun sendCommand(serverName: String, command: String): Boolean {
        return try {
            jedisPool?.resource?.use { jedis ->
                ...
                plugin.log_warning("Redis 未连接，无法发送命令")
                false
            }
        } catch (e: Exception) {
            plugin.log_error("发送命令失败: ${e.message}")
            false
        }
    }
    ...
    fun publish(type: String, data: String): Boolean {
        return try {
            ...
            plugin.log_error("发布消息失败: ${e.message}")
            false
        }
    }
